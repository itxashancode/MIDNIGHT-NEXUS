...
export async function fetchGemmaFunctionCall(
  systemInstruction: string,
  userMessage: string,
  tools: unknown[],
  signal?: AbortSignal,
  ...
  for (const model of models) {
    for (const apiKey of apiKeys) {
      try {
        ...
        const call = candidate?.content?.parts?.find((p: { functionCall?: unknown }) => p.functionCall);

        return call?.functionCall || null;
      } catch (e: unknown) {
        if (e instanceof Error) {
